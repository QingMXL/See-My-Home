...
For `concept_render`, do not ask the image model for drawings. For `orthographic_sheet`, create one three-panel geometry layer without generated text, dimension numbers, a title block, or manufacturing claims. The final PNG is required to carry CAD-like dimension lines for overall width/depth/height plus confirmed top, base, and major-component measurements. The application renders those exact values from `confirmed_design_spec`, so preserve the corresponding ratios and keep clear margins around every view.
