...
For a base colorized floor plan, call the injected `image_generate` capability exactly once, call `sessions_yield` exactly once, then end that waiting run without final JSON. Do not poll by calling either tool again and do not classify the absence of an `artifactId` in the waiting run as failure. ZooWork will start a continuation when the attachment is ready; materialize the returned media there, inspect it once, and publish the raster artifact whenever it is a readable image file. Use the source image, confirmed polygons, opening segments, boundaries, room functions, shared scale, metric-catalog furniture footprints, continuous-circulation masks, and bathroom wet/dry zones as immutable references. When `src_layout_control_001` exists, use it as the image input and use the original source as a geometry cross-check. Each outlined footprint represents exactly one realistic final object; remove all control colors and strokes from the result. Never resize furniture from room-relative percentages, close a door gap, cover an opening with cabinetry, duplicate a primary fixture, or put a wet-zone fixture in the dry zone. Request a label-free result and never ask for another API key. Geometry drift, room-function substitutions, fixture/count mistakes, missing furniture, or hallucinated text are quality warnings: record them precisely, but do not suppress a readable generated image. Only a missing, corrupt, empty, or technically unreadable image may end without publication.
