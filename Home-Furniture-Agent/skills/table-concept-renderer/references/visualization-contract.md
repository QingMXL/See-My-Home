...
The final sheet uses CAD-like concept annotations: front width and height, side depth and height, top width and depth, plus confirmed top thickness, base inset, and available dimensions for major visible components. Dimension lines use extension lines, arrowheads, and millimetre values. Image generation is not dimensional proof: the image model produces the geometry layer without rasterized text, then the application applies the exact confirmed millimetre values from `confirmed_design_spec` and exports one final PNG. Pass/fail the geometry layer on recognizable consistency with the concept specification and adequate whitespace for the annotation rails. The product does not claim fabrication readiness.
