...
import {
  ...
  listAvailableModels,
  provisionPrivateFurnitureAgent,
  syncFurnitureSkills,
} from './provision.js';
import { assertFurnitureAgentResponse, assertFurnitureTurnRequest } from './validation.js';
...
async function main(): Promise<void> {
  ...
  if (command === 'status') {
    ...
    if (!agentId) throw new Error('ZOOWORK_FURNITURE_AGENT_ID is required');
  ...
  if (command === 'provision') {
    const state = await provisionPrivateFurnitureAgent(createClientFromEnvironment(), process.env.ZOOWORK_MODEL_ID ?? '');
    process.stdout.write(`${JSON.stringify(state, null, 2)}\n`);
    return;
  }
  if (command === 'sync-skills') {
    process.stdout.write(`${JSON.stringify(await syncFurnitureSkills(createClientFromEnvironment()), null, 2)}\n`);
    return;
  }
  if (command === 'validate-request') {
    assertFurnitureTurnRequest(readJson(argument));
    process.stdout.write('FurnitureTurnRequest is valid.\n');
    return;
  }
  if (command === 'validate-response') {
    assertFurnitureAgentResponse(readJson(argument));
    process.stdout.write('FurnitureAgentResponse is valid.\n');
    return;
  }
  process.stdout.write('Home Furniture Agent Runtime\n\nCommands:\n  models\n  status\n  provision\n  sync-skills\n  validate-request <file>\n  validate-response <file>\n');
}
