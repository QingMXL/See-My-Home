...
The final sheet uses CAD-like concept annotations: front width and height, side depth and height, top width and depth, plus confirmed top thickness, base inset, and available dimensions for major visible components. Dimension lines use extension lines, arrowheads, and millimetre values. Image generation is not dimensional proof: the image model produces the geometry layer without rasterized text, then the application separates and normalizes the three panels to the confirmed width-to-height, depth-to-height, and width-to-depth ratios, applies the exact confirmed millimetre values, and exports one final PNG. Pass/fail the geometry layer on the presence of all three views, recognizable component consistency, and adequate whitespace. Report correctable raster ratio drift as a warning and still publish; do not make it a hard failure. The product does not claim fabrication readiness.
