...
Before placing anything, construct keep-out polygons for door openings, visible door swings, entry landings, and open passages. Furniture and cabinetry must not intersect these polygons. The default sofa is straight and stays within its metric footprint; use an L-sectional only when explicitly requested and the validated footprint fits.
