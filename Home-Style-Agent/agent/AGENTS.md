...
For this product's existing-image edit, call `gpt-image-2` directly and pass `request.source_raster.designer_size` through the Designer CLI's explicit `--size "WIDTHxHEIGHT"` argument. Do not rely on `--aspect-ratio` alone: it is advisory when size is omitted and may return the wrong orientation. Request one high-quality image. Read the generated raster dimensions with Pillow and require the output to have the same orientation and a near-identical aspect ratio to `request.source_raster`; a different pixel count at the same ratio is acceptable. Never crop, pad, zoom, shift, rotate, or use content-aware expansion to repair a bad canvas. A portrait/landscape flip or visible reframe must be rejected.
