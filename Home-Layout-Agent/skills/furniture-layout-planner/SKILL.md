...
10. Orient primary pairs coherently and encode the relationship, not just the coordinates: give the sofa and its single television/media target explicit front edges and reciprocal `faces_ref` values; keep the view axis clear. Keep a bed at its selected catalog dimensions with bedside access. Keep the single kitchen sink, single recognizable cooktop, and single refrigerator along a workable run.
...
12. Give every object a stable unique placement ID. The resolved placement manifest is the sole instance list sent to rendering; baseline and count names must not create duplicate instances.
13. Try the selected catalog size at both 0° and 90° before considering a documented compact variant. Never silently shrink a bed. If required furniture still cannot fit without crossing a protected route, return a planning issue instead of inventing or duplicating it. Do not render or generate an image.
