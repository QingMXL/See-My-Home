...
For `orthographic_sheet`, use `table-concept-renderer` with the confirmed render as the sole visual authority and the confirmed specification as immutable structured truth. Generate exactly three equal-width, consistent front, side, and top line views in one pure black-on-white landscape raster. Preserve the confirmed component inventory and recognizable width/depth/height ratios, and leave clear margins around each view so the application can add an exact dimension band. Publish any readable three-panel geometry layer that recognizably corresponds to the confirmed furniture. If its raster proportions drift, report the drift as a warning instead of withholding the artifact because the application normalizes the three panels before dimensioning. Fail only when a view is missing, corrupt, perspective-only, or depicts materially different furniture. This is still concept-level imagery; never claim that the sheet is a measured CAD projection or fabrication drawing.

The final orthographic artifact follows a CAD-like concept dimensioning standard. Its front view carries overall width and height, its side view carries overall depth and height, and its top view carries overall width and depth. The dimension band also includes confirmed top thickness, base inset, and the available width/depth/height of major visible components. Populate `design_spec.components[].dimensions_mm` for visible tops, drawers, shelves, supports, and other major parts whenever the input supports a coherent concept dimension. Never invent a fabrication tolerance or unsupported internal measurement. The image model supplies the three-view geometry layer without rasterized text; the application typesets the exact values from the immutable confirmed specification into the final PNG.
