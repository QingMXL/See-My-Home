...
4. Build one English product-render prompt that states the resolved overall dimensions, component arrangement, materials, finish, camera viewpoint, and prohibited changes. With a sketch, match its viewing angle, elevation, visible faces, and framing according to its source weight.
...
7. Publish the raster only when it is readable, recognizably the requested table family, and consistent with the validated viewpoint, major components, and proportions. Missing or extra drawers, shelves, supports, or hardware fail QA.
