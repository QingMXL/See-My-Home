  ...
  const model: HomeModel = {
    ...
    constraints: [
      ...
      ...renderPlan.keepout_zones.map((zone, index) => ({
        id: `constraint_keepout_${index + 1}`, category: 'physical',
        statement: `${zone.opening_ref ?? zone.id} has a ${zone.reason} keep-out zone of ${zone.clearance_mm} mm in ${zone.space_refs.join(', ') || 'adjacent spaces'}; no furniture, fixture, appliance, or cabinetry may overlap it.`,
        strength: 'hard', status: 'active', state: 'inferred', confidence: renderPlan.scale.confidence,
        source_refs: [sourceId],
      })),
      ...renderPlan.functional_zones.map((zone, index) => ({
        id: `constraint_functional_zone_${index + 1}`, category: 'physical',
        statement: `${zone.space_ref} uses ${zone.kind}; keep the single shower or tub in the wet zone and the single toilet plus vanity in the dry zone.`,
        strength: 'hard', status: 'active', state: 'inferred', confidence: renderPlan.scale.confidence,
        source_refs: [sourceId],
      })),
