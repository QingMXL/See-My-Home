...
The resolved placement manifest is the single object-instance authority for rendering. Room-program baseline and count names describe the same resolved instances and never cause another copy. Try each selected catalog size at 0° and 90° before using an explicitly allowed compact variant. Beds are never silently shrunk: a 2000 mm bed length stays about 2.35 times an estimated 850 mm reference door in the control image and generated result.
