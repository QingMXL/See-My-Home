    ...
    const runtimeEnvelope = {
      ...
      output_requirement:
        request.operation === 'intake'
          ...
          : request.operation === 'visualize'
            ? 'In this request, produce an evidence-backed diagnosis and visualization brief from the authoritative confirmed Home Model. The Runtime has already calibrated one shared plan scale, created one authoritative single-instance placement manifest, derived 900 mm continuous-circulation keep-out paths, assigned sofa-to-TV facing relationships, separated each full bathroom into wet and dry functional zones, and clipped one material zone to each confirmed room polygon. Treat those placements, physical dimensions, openings, functional zones, material zones, relationships, and hard constraints as authoritative image guidance. Room-program baseline names and counts describe the same resolved placement instances; never render them as additional copies. When sources[src_layout_control_001] exists, use its asset_ref as the image_generate image reference: it contains the original plan plus deterministic guides. Convert each uniquely shaped and colored furniture footprint into exactly one realistic top-down object. Preserve every cyan door/open-passage segment as an unfilled gap, every thinner blue window segment as a window, the purple dashed sofa-to-TV axis as an unobstructed viewing relationship, and each outlined material polygon as a hard finish boundary. Remove all guide colors, tints, dashed paths, and control strokes from the final pixels. Cross-check sources[src_floor_plan_001] for immutable walls, columns, windows, and openings. Never resize a bed: a planned 2000 mm bed length must remain visually about 2.35 times an estimated 850 mm reference-door span. Never enlarge a sofa beyond its footprint, close a door gap, or place cabinetry across an entrance or circulation path. Render exactly one television screen for a television/media target; an optional low console is part of that same target, not another screen. In every full bathroom keep exactly one toilet and one vanity in the dry zone and exactly one shower or tub in the wet zone. In every kitchen keep exactly one sink, one visibly recognizable cooktop, and one refrigerator along the planned work run. Confine every floor texture to its material-zone polygon without bleeding into adjacent spaces. Diagnosis.assessment_items must contain at most five concise bilingual design observations and may discuss only circulation, functional relationships, adjacency, privacy, daylight, storage demand, activity conflict, or underused space. Furniture, fixture, appliance, sanitary-equipment, typography, and render defects are generation-quality observations and must never appear as design assessment items. Apply explicit user_overrides or the current user request with highest priority, but otherwise treat the placement manifest as the sole object list. Preserve each confirmed room function and request a label-free result. Set visualization_brief.preferred_providers exactly to ["Banana Pro", "Image 2"]. Prefer Banana Pro for source-referenced geometry-preserving image-to-image work and Image 2 for clean-plan generation or pre-generation fallback. Use only model-selection fields that the current image tool schema actually exposes; otherwise omit model and provider so ZooWork applies its configured route. Call image_generate exactly once with action="generate", image when supported, prompt, quality="high", filename="<home_id>_<request_id>_layout.png", and aspectRatio="4:3". Do not call action="list", action="status", or launch a retry after generation starts. After the background task starts, call sessions_yield exactly once and end that run with one brief waiting sentence. ZooWork will automatically start a continuation run when the attachment arrives. Only in that continuation, call media_materialize exactly once with the returned async attachment artifactId and path="/workspace/artifacts/<home_id>/<home_id>_<request_id>_layout.png", then inspect that materialized image once. If the materialized file is a readable raster image, always call artifact_publish exactly once with that same path. Geometry drift, changed room functions, incompatible fixtures, count mismatches, missing furniture, styling weaknesses, and hallucinated or illegible text must be listed precisely in warnings but must not suppress publication. Return status="completed" with both diagnosis and visualization_brief populated after publication. Only a missing, corrupt, empty, or technically unreadable image may remain unpublished and return status="failed". Do not call any provider API or request another key.'
