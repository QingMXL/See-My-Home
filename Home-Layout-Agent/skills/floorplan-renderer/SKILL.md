...
3. Use the validated placement plan, its shared scale, physical furniture dimensions, opening keep-out polygons, and every `room_program` to add realistic top-down furniture and fixtures. Treat `baseline_objects` as soft first-draft defaults, conditional objects as space-dependent, and explicit user instructions as highest priority. After overrides, render every `default_object_counts` object within its resolved min/max count. Never enlarge furniture beyond its planned footprint or place cabinetry across an entrance.
