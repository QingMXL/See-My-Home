...
5. Build one English prompt for the requested mode. A product-render prompt states the dimensions, components, materials, finish, camera viewpoint, and prohibited changes. An orthographic prompt requests one landscape raster with exactly three consistent line-art panels: front, side, and top.
...
For `concept_render`, do not ask the image model for drawings. For `orthographic_sheet`, create one three-panel drawing image without written labels, dimension numbers, a title block, or manufacturing claims.
