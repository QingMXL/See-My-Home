...
2. Build the prompt from the Runtime's authoritative single-instance placement manifest. Treat `room_program` baseline and count names as explanations and validation targets for those same instances, never as additional object requests. Apply `user_overrides` and the current request with highest priority. Preserve metric bed footprints, the sofa-to-single-TV facing axis, exact kitchen/bathroom primary-fixture counts, and per-room material-zone clipping. Set `preferred_providers` to `["Banana Pro", "Image 2"]`: prefer Banana Pro for source-referenced image-to-image work and Image 2 for clean-plan generation or pre-generation fallback. Use only tool arguments exposed in the current ZooWork run. Do not call a provider HTTP API or ask for another key.
